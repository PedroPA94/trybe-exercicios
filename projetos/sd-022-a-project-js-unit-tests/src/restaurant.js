...
const createMenu = (myMenu) => ({ 
  ...
  pay() {
    ...
    for (let item of this.consumption) {
      if (Object.keys(menu[0]).includes(item)) {
        payTotal += menu[0][item];
      }
      if (Object.keys(menu[1]).includes(item)) {
        payTotal += menu[1][item];
      }
    }
    return payTotal + payTotal * 0.1;
  },
});
...
console.log(arrayGenerator('keys', {
  food: { coxinha: 3.90, sanduiche: 9.90 },
  drinks: { agua: 3.90, cerveja: 6.90 },
}))